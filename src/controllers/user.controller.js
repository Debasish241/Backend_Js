...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Soemthing went wrong for generating refersh token"
    );
  }
};
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  // send new tokens and refresh token
  ...
  try {
    ...
    if (!user) {
      throw new ApiError(401, "invalid refreshTken");
    }
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiError(200, channel[0], "Channel profile fetched successfully"));
});
export {
  registerUser,
